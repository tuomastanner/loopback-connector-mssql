...
MsSQL.prototype.query = function (sql, optionsOrCallback, callback) {
  ...
  var retry = function(err) {
    if (err && (err.code === 'ELOGIN' || err.code === 'ENOCONN' || err.code === 'ECONNCLOSED') && retries < maxRetries) {
      retries++;
      debug('Retrying to connect #%d/%d', maxRetries);
      self.client = null;
  ...
  var doQuery = function() {
    var innerCB = function (err, data) {
      ...
      if (err && (err.code === 'ELOGIN' || err.code === 'ENOCONN' || err.code === 'ECONNCLOSED')) {
        setTimeout(function () {
            retry(err);
        }, 100);
      }
    ...
    var request = new mssql.Request(self.client);
...
MsSQL.prototype.create = function (model, data, callback) {
  ...
  var sql = "DECLARE @ids TABLE (id " + this.columnDataType(model, modelPKID) + ");";
  sql += "INSERT INTO " + tblName + columns + MsSQL.newline;
  sql += "OUTPUT INSERTED." + this.columnEscaped(model, modelPKID)
    + " INTO @ids" + MsSQL.newline;
  sql += (fieldsAndData.paramPlaceholders ?
    "VALUES (" + fieldsAndData.paramPlaceholders + ");" :
    "DEFAULT VALUES;");
  sql += "SELECT id as insertId FROM @ids;"
  ...
  Object.keys(data).forEach(function (key) {
    if (props[key]) {
      ...
      if (key !== "id" && key !== modelPKID) {
        ...
        var prefix = '';
        if ((self.columnDataType(model, key).toLowerCase().indexOf('nvarchar') >= 0 || self.columnDataType(model, key).toLowerCase().indexOf('ntext') >= 0) && data[key]) {
            prefix = 'N';
        }
        fieldValuesPlaceholders.push(prefix + "(?)");
        combined.push(columnName + "=" + prefix + "(?)");
      }
    }
  });
  var sql = "";
  if (id) {
    self.exists(model, id, function (err, yn) {
      ...
      if (yn) {
        ...
        fieldValues.push(self.toDatabase(props[modelPKID], id, true));
      } else {
    ...
    //insert
    sql = "INSERT INTO " + tblName + columns + MsSQL.newline;
    var values = fieldValuesPlaceholders.length ?
      "VALUES (" + fieldValuesPlaceholders.join() + ");" :
      "DEFAULT VALUES;";
    sql += values + MsSQL.newline;
...
MsSQL.prototype.exists = function (model, id, callback) {
  ...
  this.query(sql, [escape(id)], function (err, results) {
...
MsSQL.prototype.destroy = function (model, id, cb) {
  ...
  this.query(sql, [escape(id)], function (err, data) {
    if (err) {
      return cb && cb(err);
    }
    cb && cb(null);
  ...
  Object.keys(data).forEach(function (key) {
    if (props[key]) {
      ...
      if (key !== "id" && key !== modelPKID) {
        ...
        fieldValuesPlaceholders.push(prefix + "(?)");
        combined.push(self.columnEscaped(model, key) + "=" + prefix + "(?)");
      }
    }
  });
  var sql = "";
...
MsSQL.prototype.find = function (model, id, callback) {
  ...
  this.query(sql, [escape(id)], function (err, results) {
    ...
    callback(null, self.fromDatabase(model, results[0]));
  });
};
...
MsSQL.prototype.buildInsert = function (model, data) {
  ...
  var props = this._models[model].properties;
  var modelPKID = this.idName(model);
  if (props[modelPKID].generated !== false) {
...
MsSQL.prototype.fromDatabase = function (model, data) {
  ...
  Object.keys(props).forEach(function (p) {
    ...
    if (val !== undefined) {
      ...
      if (props[p].type.name === 'Number' && val !== null) {
...
function datatype(p) {
  ...
  switch (p.type.name) {
    ...
    case 'String':
    case 'JSON':
      dt = '[nvarchar](' + (p.length || p.limit || 512) + ')';
      break;
    case 'Text':
      dt = '[text]';
      break;
